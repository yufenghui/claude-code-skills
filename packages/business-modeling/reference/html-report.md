...
| 服务边界溯源 | 左三类边界来源（合同/履约/领域）+ 右服务节点 | 实线映射边界来源；服务节点色标真伪微服务 | 不画服务间链接；与服务划分表互补（画法见下「微服务边界溯源图画法」） |
...
§9 微服务设计的**首要图产出**，回答"**服务边界从哪来**"（承载"为何如此划分服务边界"的判定依据）。与服务划分表互补：图给映射关系与聚合（结构性），表给逐服务字段（分类）。

### 必含元素

| # | 元素 | 画法 |
| --- | --- | --- |
| 1 | 三类边界来源（左列，来自 §3） | 按 **合同上下文 / 履约项 / 领域上下文** 三组纵向分区，浅色分组背板区分（合同=浅蓝 `#dbeafe`、履约=浅粉 `#fbcfe8`、领域=浅绿 `#bbf7d0`），组内列具体元素名 |
| 2 | 微服务节点（右列） | 圆角白卡（复用 §4.2 节点框），卡内服务名 + URI root |
| 3 | 边界来源映射线 | 实线（`#94a3b8`，`marker-end="url(#arr)"`）从左列元素 → 右列服务；贝塞尔走通道、不穿节点；多源汇聚到同一服务体现"按业务能力聚合" |
| 4 | 真伪微服务色标 | 服务节点边框/角标：真微服务 = 绿边（`#2e7d32`）✓；伪模式 = 灰边（`#94a3b8`）+ 角标（分布式服务 / 微工作组 / 傻服务 / 分布式单体） |

**不画**服务间超媒体链接（本节聚焦划分依据，非服务生态拓扑；RESTful 超媒体为补充视角）。

### 布局（左右映射式）

`viewBox="0 0 1040 360"`（按服务数纵向增高）：

- **左列（x≈30–300）**：三类边界来源分组背板，组内列具体元素（合同上下文名 / 履约项名 / 领域上下文名）。
- **右列（x≈700–1000）**：微服务节点；真微服务绿色帽/绿边、伪模式灰边 + 角标。
- **中间连线区**：实线从左元素右边 → 右服务左边（贝塞尔 `C`，走通道不交叉）；同一服务的多条入边汇聚到节点左侧同一点。
- **图例（底部）**：三类来源背板色 + 线型（实线=边界来源映射）+ 真伪色标。

> 填充时替换业务内容；坐标按「节点不重叠、连线走通道」调整（布局原则、合法性、自检见 `svg-layout.md`）。
...
- [ ] §9 微服务设计含**服务边界溯源图**（左三类边界来源 → 右服务 + 真伪色标），非只有服务划分表（见「微服务边界溯源图画法」）
