...
// frame-template.html 仅在为"非完整文档的内容片段"包壳时用到。
// 本插件始终推送完整的 report.html（走 isFullDocument 分支），不会触发 wrapInFrame，
// 因此模板改为按需懒加载：缺失也不影响 server 启动与完整文档的渲染。
const frameTemplatePath = path.join(__dirname, 'frame-template.html');
function loadFrameTemplate() {
  try { return fs.readFileSync(frameTemplatePath, 'utf-8'); } catch (e) { return null; }
}
...
function wrapInFrame(content) {
  const tmpl = loadFrameTemplate();
  // 没有模板（本插件不使用片段包壳）：原样返回，交由调用方按完整文档处理。
  if (!tmpl) return content;
  return renderBranding(tmpl).replace('<!-- CONTENT -->', content);
}
